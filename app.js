...
function getDados() {
  const rendas = [...document.querySelectorAll('.renda-row')].map(r => ({
    ...
    val:  parseFloat(r.querySelector('.renda-val')?.value) || 0,
  })).filter(r => r.val > 0);

  const despesas = [...document.querySelectorAll('.desp-row')].map(r => ({
    desc: r.querySelector('.desp-desc')?.value || '',
    val:  parseFloat(r.querySelector('.desp-val')?.value) || 0,
  })).filter(d => d.val > 0);

  const dividas = [...document.querySelectorAll('.divida-row')].map(r => ({
    credor:  r.querySelector('.div-credor')?.value  || '—',
    saldo:   parseFloat(r.querySelector('.div-saldo')?.value)   || 0,
    juros:   parseFloat(r.querySelector('.div-juros')?.value)   || 0,
    parcela: parseFloat(r.querySelector('.div-parcela')?.value) || 0,
  })).filter(d => d.saldo > 0);
...
function adicionarRenda() {
  ...
  div.className = 'renda-row';
  div.innerHTML = `
    <div class="field"><input type="text" placeholder="Ex: Salário CLT" class="renda-desc"></div>
    <div class="field"><input type="number" placeholder="0,00" min="0" step="0.01" class="renda-val"></div>
    <button class="btn-remove" onclick="this.closest('.renda-row').remove()"><i class="ti ti-x"></i></button>`;
  el.appendChild(div);
}

function adicionarDesp() {
  const el = document.getElementById('despesas-list');
  const div = document.createElement('div');
  div.className = 'desp-row renda-row';
  div.innerHTML = `
    <div class="field"><input type="text" placeholder="Ex: Aluguel" class="desp-desc"></div>
    <div class="field"><input type="number" placeholder="0,00" min="0" step="0.01" class="desp-val"></div>
    <button class="btn-remove" onclick="this.closest('.desp-row').remove()"><i class="ti ti-x"></i></button>`;
  el.appendChild(div);
}
...
function adicionarDivida() {
  ...
  const div = document.createElement('div');
  div.className = 'divida-row';
  div.innerHTML = `
    <div class="field"><input type="text" placeholder="Ex: Cartão Nubank" class="div-credor"></div>
    <div class="field"><input type="number" placeholder="0,00" min="0" step="0.01" class="div-saldo"></div>
    <div class="field"><input type="number" placeholder="0,00" min="0" step="0.1" class="div-juros"></div>
    <div class="field"><input type="number" placeholder="0,00" min="0" step="0.01" class="div-parcela"></div>
    <button class="btn-remove" onclick="this.closest('.divida-row').remove()"><i class="ti ti-x"></i></button>`;
  el.appendChild(div);
}
...
function limparTudo() {
  ...
  adicionarRenda(); adicionarDesp(); adicionarDivida();
...
function gerarEstrategia() {
  ...
  const cardsHTML = cardsData.map(c => `
    <div class="est-card ${c.id === recomendada ? 'recommended' : ''} ${c.id === recomendada ? 'selected' : ''}" id="est-${c.id}" onclick="selecionarEst('${c.id}')">
  ...
  const ordemHTML = dividasPriorizadas.map((div, i) => `
    <div class="ordem-item">
      ...
      <div style="font-weight:600;color:var(--red);min-width:90px;text-align:right;">${fmt(div.saldo)}</div>
...
function gerarRelatorio() {
  ...
  const tabelaDividas = d.dividas.map((div, i) => `
    <tr>
      <td>${div.credor}</td>
  ...
  document.getElementById('relatorio-content').innerHTML = `
    ...
    <div class="relatorio-preview" id="relatorio-doc">
      ...
      <div class="rel-metricas">
        <div class="rel-metrica">
          <div class="rel-metrica-val txt-green">${fmt(d.totalRenda)}</div>
          <div class="rel-metrica-label">Renda mensal</div>
        </div>
        <div class="rel-metrica">
          <div class="rel-metrica-val txt-red">${fmt(d.totalDividas)}</div>
          <div class="rel-metrica-label">Total em dívidas</div>
        </div>
        <div class="rel-metrica">
          <div class="rel-metrica-val" style="color:${d.comprometimento>30?'var(--red)':'var(--green)'};">${pct(d.comprometimento)}</div>
          <div class="rel-metrica-label">Renda comprometida</div>
        </div>
      </div>
      ...
      <div class="rel-metricas" style="margin-bottom:14px;">
        <div class="rel-metrica">
          <div class="rel-metrica-val txt-gold">${simAtual.meses} meses</div>
          <div class="rel-metrica-label">Prazo estimado</div>
        </div>
        <div class="rel-metrica">
          <div class="rel-metrica-val txt-red">${fmt(simAtual.jurosTotal)}</div>
          <div class="rel-metrica-label">Juros a pagar</div>
        </div>
        <div class="rel-metrica">
          <div class="rel-metrica-val" style="color:${sl>0?'var(--green)':'var(--red)'};">${fmt(sl)}</div>
          <div class="rel-metrica-label">Saldo livre/mês</div>
        </div>
      </div>
